{ stdenv, fetchurl, lzip, guileSupport ? false, pkgconfig ? null , guile ? null }:
...
stdenv.mkDerivation {
  ...
  src = fetchurl {
    url = "mirror://gnu/make/make-${version}.tar.lz";
  ...
  nativeBuildInputs = [ lzip ] ++ stdenv.lib.optionals guileSupport [ pkgconfig ];
